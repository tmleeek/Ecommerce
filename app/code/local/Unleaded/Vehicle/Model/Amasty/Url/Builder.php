...
class Unleaded_Vehicle_Model_Amasty_Url_Builder extends Amasty_Shopby_Model_Url_Builder {
    ...
    protected function getBasePart($paramPart) {
        ...
        if ($this->isVehicleSearch()) {
            if ($this->getCategoryObject()->getId() != 2 && $this->getCategoryObject()->getUrl()) {
                $url = $this->getCategoryObject()->getUrl();
            } else {
                $path = explode("/", Mage::app()->getRequest()->getPathInfo());
                unset($path[0]);
                $properPath = implode("/", $path);
                $url = $base . $properPath;
            }
        } elseif ($this->isCatalogSearch()) {
    ...
    protected function isVehicleSearch() {
        $isVehicleSearch = $this->moduleName == 'vehicle';
        return $isVehicleSearch;
    }
