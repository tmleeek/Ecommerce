...
class Developer_acc
{
	...
	*/
	function set_sections()
	{
        ...
        // global variables
        $query = $this->EE->template_model->get_global_variables();

        $html .= '<li class="parent"><a href="'.BASE.'&C=design&M=global_variables" tabindex="-1">Global Variables</a><ul>';

        foreach ($query->result() as $row)
        {
            $html .= '<li><a href="'.BASE.'&C=design&M=global_variables_update&variable_id='.$row->variable_id.'" tabindex="-1">'.$row->variable_name.'</a></li>';
        }

        $html .= '<li><a href="'.BASE.'&C=design&M=global_variables_create" tabindex="-1" class="add">Add Global Variable</a></li></ul></li>';
        ...
        $this->EE->load->library('template');
				$EE_Template = new EE_Template();
        $EE_Template->fetch_addons();
        $plugins = $EE_Template->plugins;
        ...
        if (count($plugins))
        {
            $html .= '<li class="parent"><a href="'.BASE.'&C=addons_plugins" tabindex="-1">Plugins</a><ul>';

            foreach ($plugins as $plugin)
            {
                $html .= '<li><a href="'.BASE.'&C=addons_plugins&M=info&name='.strtolower($plugin).'" tabindex="-1">'.ucwords(str_replace('_', ' ', $plugin)).'</a></li>';
            }
		...
		<style type="text/css">
			...
			#developer_acc li.subparent {background: url('.$this->EE->config->slash_item('theme_folder_url').'cp_themes/default/images/nav_arrow_light.png) center right no-repeat;}
