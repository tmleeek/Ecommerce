...
{	
	...
	*/
	function results($reset_query=true) 
	{
		...
		//$request  = 'https://api.twitter.com/1.1/search/tweets.'.$this->type;
		...
		//get the results
	    $_res = $this->process($request);

	    //cURL not loaded?
		if($_res == 'curl_not_loaded')
		{
			return $_res;
		}

		return $this->objectify($_res)->results;
	}
