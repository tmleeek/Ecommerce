...
<form method="post" action="<?=$base_url?>&amp;method=save_collection" class="low-form">
	...
	<table class="mainTable" cellspacing="0" cellpadding="0">
		...
		<tbody>
			...
			<tr class="<?=low_zebra()?>">
				<td>
					<label for="collection_language"><?=lang('collection_language')?></label>
					<p><?=lang('collection_language_notes')?></p>
				</td>
				<td>
					<select name="language" id="language">
						<option value="">--</option>
						<?php foreach ($languages AS $label => $value): ?>
							<option value="<?=$value?>"<?php if ($value == $language): ?> selected="selected"<?php endif; ?>>
			...
			<tr class="<?=low_zebra()?>">
				...
				<td><input class="small" name="modifier" id="collection_modifier" decimal="." size="10" type="number" min="0.1" max="9999" step="0.1" value="<?=$modifier?>" /></td>
			</tr>
		</tbody>
	</table>
