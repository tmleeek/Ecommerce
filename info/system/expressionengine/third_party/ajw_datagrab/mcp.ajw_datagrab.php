...
class Ajw_datagrab_mcp {
	...
	*/
	...
	function index() {
		...
		$this->EE->db->select('id, name, description, passkey, import_status, import_lastrecord, import_totalrecords, last_run, settings');
		...
		foreach($query->result_array() as $row) {
			...
			if( $row["import_status"] == "WAITING" ) {
				$row[] = "Waiting - " . $row["import_lastrecord"] . " out of " . $row["import_totalrecords"] . " records (" . round($row["import_lastrecord"]/$row["import_totalrecords"]*100) . "%)" . 
					' <a href="' . DATAGRAB_URL.AMP.'method=run'.AMP.'id='.$row["id"].AMP."batch=yes" . '">Next batch</a>';
			} elseif ( $row["import_status"] == "RUNNING" ) {
				$row[] = "Running - " . $row["import_lastrecord"] . " out of " . $row["import_totalrecords"] . " records (" . round($row["import_lastrecord"]/$row["import_totalrecords"]*100) . "%)";
			} else {
				$row[] = $row["import_status"];
			}
			$row[] = $this->EE->localize->human_time( $row["last_run"] );
	...
	function configure_import() {
		...
		$this->EE->db->select("channel_title, field_group, cat_group");
		...
		// Transcribe module
		if (array_key_exists('transcribe', $this->EE->addons->get_installed('modules'))) {
			$this->EE->db->select( "id, name" );
			$query = $this->EE->db->get( "exp_transcribe_languages" );
		...
		// Nsm Better Meta
		if (array_key_exists('nsm_better_meta', $this->EE->addons->get_installed('modules'))) {
		...
		// 	
		$data["datatype_info"] = $this->EE->datagrab->datatypes[ $this->settings["import"]["type"] ]->datatype_info;
		$data["datatype_settings"] = $this->EE->datagrab->datatypes[ $this->settings["import"]["type"] ]->settings;
	...
	function import() {
		...
		if( $this->EE->input->get('batch') !== FALSE ) {
			$this->settings["import"]["batch"] = $this->EE->input->get('batch');
		}
		...
		$data["batch_action"] = DATAGRAB_URL.AMP.'method=import&batch=yes';
		...
		// Load view
		$this->EE->load->remove_package_path(PATH_THIRD.'ep_better_workflow/'); // Fixes issue if EP Better Workflow installed
		return $this->EE->load->view('_wrapper', $data, TRUE);
	...
	function run() {
		...
		if ( $this->EE->input->get( "batch" ) == "yes") {
			$this->EE->functions->redirect(DATAGRAB_URL.AMP."method=import&batch=yes");
		} else {
	...
	function _get_input() {
		...
		switch( $datagrab_step ) {
			...
			case "configure_import": {
				...
				if (array_key_exists('transcribe', $this->EE->addons->get_installed('modules'))) {
					$this->settings["config"][ "ajw_transcribe_language" ] = $this->EE->input->post( 'ajw_transcribe_language' );
					$this->settings["config"][ "ajw_transcribe_related_entry" ] = $this->EE->input->post( 'ajw_transcribe_related_entry' );
				}
				...
				if (array_key_exists('nsm_better_meta', $this->EE->addons->get_installed('modules'))) {
					$this->settings["cf"][ "ajw_nsm_better_meta_title" ] = $this->EE->input->post( 'ajw_nsm_better_meta_title' );
					$this->settings["cf"][ "ajw_nsm_better_meta_keywords" ] = $this->EE->input->post( 'ajw_nsm_better_meta_keywords' );
					$this->settings["cf"][ "ajw_nsm_better_meta_description" ] = $this->EE->input->post( 'ajw_nsm_better_meta_description' );
				}
